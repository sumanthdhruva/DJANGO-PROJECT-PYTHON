...
def randomlogic(request):
    if request.method == "POST":
        user_input = request.POST.get('user_input')  # Use .get() for safer access

        # Validate user input (optional, adjust error message as needed)
        try:
            a2 = int(user_input)
            if a2 <= 0:
                error = "OTP length must be a positive number."
                return render(request, 'randomotpgenerator.html', {'error': error})
        except ValueError:
            error = "Invalid OTP length. Please enter a number."
            return render(request, 'randomotpgenerator.html', {'error': error})

        # Limit OTP length (optional, set max_otp_length as needed)
        max_otp_length = 10
        a2 = min(a2, max_otp_length)
    ...
    # You might want to display an initial message in the template
    # for the first visit before any OTP generation
    return render(request, 'randomotpgenerator.html')
...
def feedbackform(request):
    if request.method == 'POST':
        ...
        tosend = feedback + 'Thank you'

        try:
            existing_feedback = Feedback.objects.get(email=email)
            # If feedback with the same email exists, handle it accordingly
            # For example, you can update the existing feedback or return an error message
            return HttpResponse("Feedback with this email already exists.")
        except ObjectDoesNotExist:
            # If feedback with the same email doesn't exist, create a new one
